...
fn generate_test_script(interface: &fgrnd_plugin_sdk::PluginInterfaceSchema, interface_name: &str) -> String {
    ...
    for cap_ref in interface.caps.iter() {
        let cap_name = match cap_ref {
            fgrnd_plugin_sdk::CapReference::Inline(cap) => &cap.cap.name,
            fgrnd_plugin_sdk::CapReference::Reference { cap_ref } => {
...
fn generate_test_config(interface: &fgrnd_plugin_sdk::PluginInterfaceSchema) -> String {
